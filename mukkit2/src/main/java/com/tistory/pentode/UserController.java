...
public class UserController {
	...
	public String updateUser(@ModelAttribute("userVO") UserVO userVO, Model model, HttpSession session, RedirectAttributes rttr) throws Exception {
		if(userVO.getNewnickname()== null || userVO.getNewnickname()== "") {
			userVO.setNewnickname(userVO.getNickname());
		}
		if(userVO.getNewpassword()== null || userVO.getNewpassword()== "" ) {
			userVO.setNewpassword(userVO.getPassword());
		}
	...
	public String findPwd(@ModelAttribute("userVO") UserVO userVO) throws Exception{
		return "user/findPwdForm";
	}

	@RequestMapping(value = "/findPwdSuccess", method = { RequestMethod.GET, RequestMethod.POST })
	public String findPwdSuccess(@ModelAttribute("userVO") UserVO userVO, Model model) throws Exception{
	...
	public ResponseEntity<String> uploadAjax(MultipartFile file, HttpSession session) throws Exception {
		...
		//userService.insertImg(vo.getMemberid(), File.separator + file.getOriginalFilename());
	...
	public ResponseEntity<byte[]> displayFile(String fileName, HttpSession session) throws Exception {
		...
		UserVO vo = (UserVO) session.getAttribute("login");
		userService.insertImg(vo.getMemberid(), fileName);
    ...
    public int checkId(String memberid, ModelAndView mav) throws Exception {
        System.out.println("Controller.idCheck() 호출");
        System.out.println(memberid);
        int result=0;
        result = userService.checkId(memberid);
        return result;
    }
	
	@ResponseBody
    @RequestMapping(value="/checkNickname")
    public int checkNickname(String nickname, ModelAndView mav) throws Exception {
    ...
    public int checkPwd(String password, ModelAndView mav, HttpSession session) throws Exception {
        System.out.println("Controller.checkPwd() 호출");
        UserVO user = (UserVO)session.getAttribute("login");
        int result=0;
        result = userService.checkPwd(user.getMemberid(), password);
        return result;
    }
	...
	@RequestMapping(value="/sendmail", method= {RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String joinPost(String memberid, Model model) throws Exception {
		logger.info("currnent join member: " + memberid);
			String test = userService.sendmail(memberid);
			System.out.println("sendmail");
	...
	@ResponseBody
	public String emailConfirm(String uid) throws Exception {
		System.out.println("joincheck controller() 호출");
		uVO.setMemberscore(0);
		uVO.setMemberlv("Rookie");	// authstatus를 1로,, 권한 업데이
